...
// AFTER Stop, leaving a stopped dashboard's server running (and serving the
// dead handlers). Stop must mark the dashboard stopped so a late startServer
// is refused.
func TestUnloadPreventsRestart(t *testing.T) {
	...
	// the server.
	if err := m.startServer(); err == nil {
		t.Fatal("startServer after Stop should refuse (stopped flag)")
	}
	if m.isRunning() {
