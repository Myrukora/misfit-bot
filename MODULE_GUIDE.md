...
- **Always set `Description`** — it's the text that appears after `—` in `[p]help`. A blank `Description` shows as `?name —` (blank), which is what the user sees for poorly-written modules. The dashboard module's command had this bug and was fixed. Write it as a plain, user-friendly sentence of what the command does (keep it ≤ 100 chars — it also becomes the slash command description).
